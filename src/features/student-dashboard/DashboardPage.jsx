          ...
          <div className="sidebar-card">
            ...
            <div>
              {studentData.interests.map(i => (
                <span key={i} className="interest-tag" style={{cursor:"pointer"}} onClick={() => navigate("/courses")} title={t("dash.browse")}>{i}</span>
              ))}
            </div>
          </div>
          <div className="sidebar-card">
        ...
        <div className="dash-main">
          <div style={{ background: "var(--surface)", border: "1px solid var(--border2)", borderRadius: 14, padding: "1.5rem" }}>
            ...
            <div className="enrolled-list">
              {enrolled.map(c => (
                <div key={c.id} className="enrolled-card" style={{cursor:"pointer"}} onClick={() => navigate(`/courses/${c.id}`)}>
          ...
          <div style={{ background: "var(--surface)", border: "1px solid var(--border2)", borderRadius: 14, padding: "1.5rem" }}>
            ...
            <div className="rec-list">
              {recommended.map(c => (
                <div key={c.id} className="rec-card" style={{cursor:"pointer"}} onClick={() => navigate(`/courses/${c.id}`)}>
